...
watch(() => state.value.themeName, (newName) => {
    ...
    <Transition name="modal">
      <div v-if="show" class="modal-overlay" @click="handleClose">
        <div class="modal-content" @click.stop :style="{
          background: state.mode === 'dark' ? 'rgba(15, 15, 20, 0.98)' : 'rgba(250, 250, 252, 0.98)',
          borderColor: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)',
          color: state.mode === 'dark' ? '#fff' : '#000'
        }">
          <div class="modal-header" :style="{
            borderBottomColor: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.15)' : 'rgba(0, 0, 0, 0.15)'
          }">
            <h2 :style="{ color: state.mode === 'dark' ? '#fff' : '#000' }">Export Theme</h2>
            <button @click="handleClose" class="close-btn" :style="{
            ...
            <div class="form-group">
              <div class="form-label-row">
                ...
                <div class="quick-actions">
                  <button @click="selectAll" class="quick-btn" :style="{
                    color: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.6)' : 'rgba(0, 0, 0, 0.6)'
                  }">
                    Select All
                  </button>
                  <button @click="deselectAll" class="quick-btn" :style="{
                    color: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.6)' : 'rgba(0, 0, 0, 0.6)'
                  }">
                    Clear
                  </button>
              ...
              <div class="format-grid">
                <label
                  ...
                  :style="{
                    background: selectedFormats.includes(format.id)
                      ? (state.mode === 'dark' ? 'rgba(100, 150, 255, 0.15)' : 'rgba(100, 150, 255, 0.1)')
                      : (state.mode === 'dark' ? 'rgba(255, 255, 255, 0.03)' : 'rgba(0, 0, 0, 0.03)'),
                    borderColor: selectedFormats.includes(format.id)
                      ? (state.mode === 'dark' ? 'rgba(100, 150, 255, 0.5)' : 'rgba(100, 150, 255, 0.4)')
                ...
                >
                  ...
                  <div class="format-info">
                    ...
                    <div class="format-desc" :style="{
                      color: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)'
                    }">
                      {{ format.desc }}
                    </div>
                  </div>
                  <div v-if="selectedFormats.includes(format.id)" class="check-mark" :style="{
                    color: state.mode === 'dark' ? 'rgba(100, 150, 255, 1)' : 'rgba(100, 150, 255, 1)'
                  }">
                    ✓
                  </div>
                </label>
          ...
          <div class="modal-footer" :style="{
            borderTopColor: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.15)' : 'rgba(0, 0, 0, 0.15)'
          }">
            <button @click="handleClose" class="btn-cancel" :style="{
              borderColor: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)',
              color: state.mode === 'dark' ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)'
            }">
              Cancel
            ...
            <div class="export-buttons">
              <button @click="handleExport('dark')" class="btn-export btn-dark" :style="{
              ...
              <button @click="handleExport('light')" class="btn-export btn-light" :style="{
              ...
              <button @click="handleExport('both')" class="btn-export btn-both" :style="{
